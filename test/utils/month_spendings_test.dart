...
void main() {
  test('return total monthly spending for a user', () {
    final now = DateTime.now();

    final fakeBox = FakeExpenseBox([
      Expense(userID: 'user1', amount: 100, date: now, categoryID: 1),
      Expense(
        userID: 'user1',
        amount: 50,
      ...
      Expense(userID: 'user2', amount: 200, date: now, categoryID: 3),
    ]);

    final total = monthSpendings('user1', fakeBox as dynamic);

    expect(total, 100);
  });

  test('return 0 if user has no expenses this month', () {
    final now = DateTime.now();
    ...
    final total = monthSpendings('user2', fakeBox as dynamic);
